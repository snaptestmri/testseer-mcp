...
export const externalEndpointsTool = {
  ...
  description:
    "Query indexed external/partner HTTP endpoints for a service (external API, OIS callbacks, etc.). " +
  ...
  inputSchema: {
    ...
    properties: {
      ...
      partner: { type: "string", description: "Partner slug e.g. webhook, example" },
      flowStep: { type: "string", description: "Flow step e.g. HANDLER_A" },
    },
    required: ["serviceId"],
  },
};
